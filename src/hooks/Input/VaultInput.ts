...
export function useVaultInput(token: Token, vault: string, library: any, onChange?: (data: number) => void, defaultValue?: number) {
    ...
    useEffect(() => {
        (async () => setApproved(await isApproved(token.address, vault, library.getSigner())))();
    }, []);
